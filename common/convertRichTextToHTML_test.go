...
func Test_RichTextToHTML(t *testing.T) {
	type args struct {
		givenInput  []excelize.RichTextRun
		expFilepath string
		expErr      error
	}
	tcs := map[string]args{
		"success": {
			...
			expFilepath: "testdata/description.html",
		},
	}
	...
	for scenario, tc := range tcs {
		...
		t.Run(scenario, func(t *testing.T) {
			...
			// When
			rs := RichTextToHTML(tc.givenInput)

			// Then
			f, err := os.Open(tc.expFilepath)
			require.NoError(t, err)
			defer f.Close()

			expResult, err := io.ReadAll(f)
			require.NoError(t, err)

			require.Equal(t, string(expResult), rs)
		})
	}
}

func Benchmark_RichTextToHTML(b *testing.B) {
	givenInput := []excelize.RichTextRun{
		{
			Font: &excelize.Font{
				Bold:         true,
